# Warcraft Logs Provider Design Notes (history)

Design record and schema research captured while `warcraftlogs` was built. Current behavior lives in
[docs/warcraftlogs/README.md](../../warcraftlogs/README.md); the planning language below is kept for
provenance and is not a backlog. Command families listed as targets here were either shipped, renamed,
or dropped -- trust the consumer README and `warcraftlogs --help`, not this file.
...
Use `warcraftlogs graphql` when an official GraphQL query is needed before a typed command exists. It preserves the same auth routing, cache opt-in, partial-error warnings, and JSON envelope as typed commands; see [SCOPING.md](../../warcraftlogs/SCOPING.md#raw-graphql) for variable and helper rules.
...
[CACHING.md](../../warcraftlogs/CACHING.md).
...
[CACHING.md](../../warcraftlogs/CACHING.md). Freshness is surfaced in output because ranking data, report
...
Store synthetic GraphQL response fixtures for:
- guild lookup
- guild rankings
- character lookup
