...
- `warcraft guide-builds-simc` reads explicit embedded guide build references from one exported guide bundle or a `guide-compare-query` output root, dedupes them, and hands those exact build refs to `simc identify-build` plus optional `simc decode-build`. Guide providers publish bare `wow_talent_export` strings today, which `simc decode-build` rejects for want of a class/spec, so `--decode` over a guide bundle reports `summary.simc_handoff_status: "partial"` with `summary.empty_requested_legs: ["decode"]` — never `ok` next to a zero counter
