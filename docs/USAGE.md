...
- the merged page then applies three rules, all visible in the payload (see `docs/foundation/WRAPPER_PROVIDER_CONTRACT.md`): a bare name is not a profile query, so Raider.IO rows are marked `wrapper_ranking.off_intent` and rank below every row from a family the query asked for, while an exact entity-title match anchors the page; no single provider may take more than half the page (over-cap rows are deferred and fill the remaining slots, off-intent rows are capped at a strict minority); and a row whose own title is, or starts with, the query outranks one that merely mentions it. `data.merge_policy` reports the caps and how many rows they deferred or withheld
