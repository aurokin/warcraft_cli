...
`kind`, `schema_version`, `query`, `provenance`, and `data`. Read the payload from `data`; some
providers also copy their payload keys to the top level for older agents, and those copies are
deprecated.
