...
- only `wowhead_talent_calc_url` rows decode unaided, because the URL path names the class and spec; today's `/talents` sections publish import strings, so expect `decode_success_count` 0
