...
- the sampled commands collapse one real pull that two raiders both logged into a single kill (same encounter, difficulty, raid size and guild, with wall-clock start and end within 5 s); the collapse is reported, never silent — `sample.duplicates_removed` counts it, a note states the rule, and the kept kill's `duplicate_reports` cites the folded-in report codes and fight ids
