...
- `report-player-talents` is the first narrow build-transport lane:
  ...
  - in that validated packet the hero-tree selection node is resolved (tree `selection`, named after the hero tree) but never enters the split strings; tiered nodes, where one node spreads ranks over several entries, are listed under `validation.round_trip.tiered_nodes` and checked by node presence plus `node_max_rank`/`total_rank_within_node_capacity` only — each row carries `per_entry_ranks_verified: false`, because SimC's decode cannot say how the node's total was split across its entries; entries SimC grants from a hero tree this build did not pick are listed under `validation.round_trip.ignored_unselected_hero_entries`
...
- `report-player-details` also fails with `not_found` (exit 4) when a well-formed slice matches no fight — an unknown `--fight-id`, an `--encounter-id`/`--difficulty` the fight does not have, or a window with no fight in it — because a fight Warcraft Logs actually has always returns a roster, so an empty one means the slice missed rather than that the report has no players
